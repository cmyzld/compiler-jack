...
jn_statements copy_statements(jn_statements statements)
{
    ...
    {   
		...
		if(ast_kind_of(statements->get(i)) == k_jn_return || ast_kind_of(statements->get(i)) == k_jn_return_expr)
			break;
    }
...
jn_statement copy_if(jn_if if_s)
{
    ...
    case k_jn_if:
		{
			jn_if s = jn_if_create(cond,if_true) ;
			if(ast_kind_of(cond) == k_jn_bool && to_jn_bool(cond)->tf == true)
				return to_jn_statement(if_true);
			else
				return to_jn_statement(s);
		}
    case k_jn_if_else:
    default:
        {
			if(ast_kind_of(cond) == k_jn_bool)
			{
				if(to_jn_bool(cond)->tf == true)
					return to_jn_statement(if_true);
				return to_jn_statement(copy_statements(to_jn_if_else(if_s)->if_false));
			}
...
jn_statement copy_while(jn_while while_s)
{
	...
	if(ast_kind_of(cond) == k_jn_bool && to_jn_bool(cond)->tf == false)
