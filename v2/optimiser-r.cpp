...
jn_statements copy_statements(jn_statements statements)
{
    ...
    {   
		jn_statement stmt = copy_statement(statements->get(i));
		ast_kind kind = ast_kind_of(statements->get(i));
		if(kind == k_jn_if) //if (cond) { statements }
		{
			jn_if ifstmt = to_jn_if(stmt);
			jn_expr expr = ifstmt->cond;
			if(ast_kind_of(expr) == k_jn_bool)
			{
				if(to_jn_bool(expr)->tf == true)
					copy->append(to_jn_statement(copy_statements(ifstmt->if_true)));
			}	
			else
				copy->append(stmt);
		}	
		else if(kind == k_jn_if_else)
		{
			jn_if_else if_else = to_jn_if_else(stmt);
			jn_expr cond = if_else->cond;
			if(ast_kind_of(cond) == k_jn_bool)
			{
				if(to_jn_bool(cond)->tf == true)
					copy->append(to_jn_statement(copy_statements(if_else->if_true)));
				else
					copy->append(to_jn_statement(copy_statements(if_else->if_false)));
			}
			else
			{
				copy->append(stmt);
		...
		else if(kind == k_jn_while)
		{
			jn_while whilestmt = to_jn_while(stmt);
			if((ast_kind_of(whilestmt->cond) == k_jn_bool && to_jn_bool(whilestmt->cond)->tf == false))
				copy->append(to_jn_statement(jn_statements_create()));
			else
				copy->append(copy_statement(statements->get(i)));
		}
		else
        	copy->append(copy_statement(statements->get(i))) ; 
		if(kind == k_jn_return || kind == k_jn_return_expr)
			break;
    }
